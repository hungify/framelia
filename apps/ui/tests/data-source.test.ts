...
function run(runId: string): UIRun {
  return {
    schemaVersion: 1,
    runId,
    status: "running",
...
function uiEvent(runId: string): UIEvent {
...
describe("shouldRefreshOnRunEvent", () => {
  it("only matches the run currently displayed", () => {
    expect(shouldRefreshOnRunEvent("a", uiEvent("a"))).toBe(true);
    expect(shouldRefreshOnRunEvent("a", uiEvent("b"))).toBe(false);
    expect(shouldRefreshOnRunEvent(undefined, uiEvent("a"))).toBe(false);
  });
});

describe("connectRunEvents", () => {
  it("triggers the callback only for a 'run' event matching the current run", () => {
    let current: UIRun | undefined = run("a");
    ...
    source.emit("run", { data: JSON.stringify(uiEvent("b")) });
    expect(onMatchingRunEvent).not.toHaveBeenCalled();

    source.emit("run", { data: JSON.stringify(uiEvent("a")) });
