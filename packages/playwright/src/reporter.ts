...
function deriveContract(
  ...
  const profile = primary.profile ?? (scope.kind === "region" ? "component/strict" : "page");
  const expectedSize =
    scope.kind === "region"
  ...
  const imageEvidence: Pick<DashboardContractResult, "baseline" | "actual" | "diff"> = {
    ...(expectedPath
      ? {
          baseline: {
            path: dashboardArtifactPath(id, "figma-gold.png"),
            width: primary.goldSize.width,
            height: primary.goldSize.height,
            provenance: "figma",
          },
        }
      : {}),
    ...(actualAttachmentPath
      ? {
          actual: {
            path: dashboardArtifactPath(id, "actual.png"),
      ...
      : {}),
    ...(diffAttachmentPath ? { diff: { path: dashboardArtifactPath(id, "diff.png") } } : {}),
  };
...
function writeEvidence(
  ...
  const profile = score.profile ?? (scope.kind === "region" ? "component/strict" : "page");
  const expectedSize =
    scope.kind === "region"
  ...
  onTestEnd(test: TestCase, result: TestResult): void {
    const record = (): void => {
      ...
      for (const derived of derivedContracts) {
        ...
        try {
          writeEvidence(derived.writeEvidence, this.#options.maxMaskedAreaRatio);
