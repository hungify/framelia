...
function buildShortcuts(server: UIServer, onRestart: () => void): Shortcut[] {
  return [
    {
      key: "r",
...
async function serveUI(
  ...
  const shutdown = waitForUIShutdown();
  let shouldOpen = open;
  ...
  for (;;) {
    ...
    const server = await startUIServer({ source, hostname });
    let restart = false;
    try {
...
export async function runAggregatedUI(options: {
  projectRoot: string;
  open: boolean;
  host: string;
  hostExplicit: boolean;
  ...
  await serveUI(
    await aggregateUISource(options.projectRoot),
    options.open,
    options.host,
...
async function openCommand(
  ...
  await serveUI(
    await archivedUISource(artifact, suiteName),
    options.open,
...
async function reportCommand(options: { artifact: string; output: string }): Promise<void> {
  const artifact = await readVerificationArtifact(options.artifact);
  const suiteName = suiteNameFromArtifactPath(options.artifact);
  const indexPath = await exportUIReport({
    artifact,
    suiteName,
...
export function registerUICommands(program: Command): void {
  program.addCommand(
    subcommand("ui", `Open the UI aggregating every verification artifact under ${FRAMELIA_DIR}/.`)
      .option("--project-root <dir>", "target project root")
      .option("--host [host]", "host to bind (bare flag binds every interface)", "localhost")
      .option("--no-open", "do not open UI in browser")
      .action(
  ...
  program.addCommand(
    subcommand("open", "Open the UI for an existing verification artifact.")
  ...
  program.addCommand(
    subcommand("report", "Export a static UI report.")
      .requiredOption("--artifact <path>", "verification artifact JSON")
