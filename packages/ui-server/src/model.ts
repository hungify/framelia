...
export function summarize(contracts: UIContractResult[]): UISummary {
...
export function overallStatus(summary: UISummary): UIVerdict {
...
async function readScore(
  ...
  try {
    ...
    console.error(
      `framelia ui: failed to read ${RUN_ARTIFACT.score} in ${outDir}: ${String(error)}`,
    );
    return undefined;
  }
}
...
function deriveContractResult(input: ContractResultInput): ContractResultProjection {
  ...
  const diagnostics: UIDiagnostic[] = [
    ...
    ...(!result.ok
      ? [unmatchedRegionDiagnostic(contract, result)].filter(
          (diagnostic): diagnostic is UIDiagnostic => diagnostic !== null,
        )
      : []),
  ];
  const hasMask =
  ...
  const status: UIVerdict = deriveUIVerdict({
    resultOk: result.ok,
    pass: result.pass,
    diagnostics,
    maskApplied: hasMask,
...
export async function projectArtifact(
  ...
  const projections = await Promise.all(
    artifact.request.contracts.map(async (contract) => {
      ...
      const uiScore = await readScore(result.outDir, expectedUrl, defaults.deviceScaleFactor);
      return deriveContractResult({
        contract,
        result,
        score: uiScore?.score,
        captureEvidence: uiScore?.captureEvidence,
        rawTargetUrl,
        createdAt: artifact.createdAt,
      });
    }),
  );
  const files: UIFileMap = new Map();
