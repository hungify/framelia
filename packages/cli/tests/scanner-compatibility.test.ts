...
describe("scanner compatibility: aliases", () => {
  it.each(["fetch-gold", "diff"])(
    "`%s` routes without an 'unknown command' scanner error",
    async (alias) => {
      const fakeProcess = createFakeProcess();
...
describe("scanner compatibility: nested-route defaults", () => {
  ...
  it.each(["contract", "baseline"])(
    "`%s` route map with no `defaultCommand` prints its own help instead",
    async (routeMap) => {
      const fakeProcess = createFakeProcess();
