...
export function attributeDiffRegions(
  ...
  return clusters.map((cluster) => {
    ...
    for (const bounds of selectors) {
      if (overlaps(cluster.bbox, bounds)) matchedSelectors.push(bounds.selector);
    }
    return { bbox: cluster.bbox, pixels: cluster.pixels, selectors: matchedSelectors };
  });
}
