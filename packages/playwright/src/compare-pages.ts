...
export interface ComparePagesOptions {
   ...
   *  at 4) instead of 1. pageB must share the same `deviceScaleFactor`, or the mismatch
   *  surfaces as a compare() dimension error. */
  scale?: true;
}
  ...
  try {
    ...
    if (maskBounds.length) {
      ...
      combinedMaskEvidence = captureA.maskEvidence && {
        ...
        bounds: maskBounds,
        matchedCount: maskBounds.length,
        requested: captureA.maskEvidence.requested.map((entry, index) => ({
          ...entry,
          matchedCount:
            entry.matchedCount + (captureB.maskEvidence?.requested[index]?.matchedCount ?? 0),
        })),
        unionMaskedArea,
