import { resolveDisplayThreshold, resolveStyleGateEligible } from "../profiles.ts";
...
/**
 * The one place both dashboard paths (playwright reporter's live run, dashboard-server's
 * durable report) turn a scored contract into a DashboardContractResult.
 *
 * It owns the object's shape *and* the score-derived fields -- `comparison`,
 * `resolvedThreshold`, `styleGateEligible`. Those three used to be computed by each caller
 * from the same three helpers, in the same conditional, in two packages; a run could then
 * read one way live and another way from its artifact. Callers now hand over the score.
 */
export function assembleContractResult(
  ...
  return {
    ...
    ...(score
      ? {
          comparison: deriveComparisonSummary(score),
          resolvedThreshold: resolveDisplayThreshold(score),
          styleGateEligible: resolveStyleGateEligible(score),
        }
      : {}),
