...
async function fetchJson(url: string): Promise<UIRun> {
  const response = await fetch(url, { cache: "no-store" });
  if (!response.ok) throw new Error(`${response.status} ${response.statusText}`);
  return response.json() as Promise<UIRun>;
}

/** A live run only stops taking further updates once it has actually finished. */
export function shouldCloseEventsOnError(run: UIRun | undefined): boolean {
  return Boolean(run?.finishedAt);
}

/** Multiple runs can share one EventSource lifetime; only react to the run we're showing. */
export function shouldRefreshOnRunEvent(currentRunId: string | undefined, event: UIEvent): boolean {
