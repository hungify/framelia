...
describe("resolveDisplayThreshold", () => {
  it("displays explicit thresholds while preserving undefined defaults and a null pixel cap", () => {
    const source = {
      profile: "component/strict" as const,
      profileOverrides: { minMatch: 0.9, maxDiffPixels: null, minSSIM: undefined },
      clusterCheck: true,
    };
    expect(resolveDisplayThreshold(source)).toMatchObject({
      minMatch: 0.9,
      maxDiffPixels: null,
      minSSIM: 0.985,
      cluster: true,
