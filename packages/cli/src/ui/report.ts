...
export async function archivedUISource(
 ...
 * ones added to UIContractResult after this function was written.
 */
...
function withFeaturePrefix(projection: UIProjection, feature: string): UIProjection {
  ...
  const contracts: UIContractResult[] = projection.run.contracts.map((contract) => ({
...
export async function aggregateUISource(projectRoot: string): Promise<UISource> {
  ...
  const files = new Map<string, string>();
  const contracts: UIRun["contracts"] = [];
...
const REPORT_MARKER = "framelia-ui-report";
...
export async function exportUIReport(options: {
  artifact: VerificationArtifact;
  suiteName?: string;
  ...
  for (const [relativePath, sourcePath] of projection.files) {
    ...
    if (!destination.startsWith(`${dataRoot}${path.sep}`))
      throw new Error(`Unsafe UI artifact path: ${relativePath}`);
