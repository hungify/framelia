...
describe("suggestMasks", () => {
  ...
  it("prefers data-testid over the heuristic's own selector when present", async () => {
    ...
    expect(suggestions).toHaveLength(1);
    expect(suggestions[0]).toMatchObject({
      selector: ':is([data-dynamic])[data-testid="last-updated"]',
  ...
  it("reports the true match count (and maxMatches) when the same specific selector repeats (PR #51 review)", async () => {
    ...
    expect(suggestions).toHaveLength(1);
    expect(suggestions[0]).toMatchObject({
      selector: ':is([data-dynamic])[data-testid="row-status"]',
      matchedCount: 3,
      maxMatches: 3,
    });
  });
  ...
  it("falls back to #id when no data-testid is present", async () => {
    ...
    expect(suggestions[0]).toMatchObject({ selector: ":is([data-dynamic])#clock" });
  });

  it("scopes the specific selector to the matched heuristic so a static element sharing the same id/data-testid is excluded (PR #52 review)", async () => {
    const suggestions = await suggestFor(`
      <div data-dynamic data-testid="shared-id">Live value</div>
      <span data-testid="shared-id">Static label, not dynamic</span>
    `);
    expect(suggestions).toHaveLength(1);
    expect(suggestions[0]).toMatchObject({
      selector: ':is([data-dynamic])[data-testid="shared-id"]',
      heuristic: "data-dynamic",
      matchedCount: 1,
    });
  });
