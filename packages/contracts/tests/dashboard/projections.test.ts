...
describe("assembleContractResult", () => {
  ...
  it("derives comparison, resolvedThreshold and styleGateEligible from one score", () => {
    ...
    expect(result.comparison).toEqual({
      ...
      diffPixels: 12,
      diffRatio: expect.closeTo(0.01, 5),
      matchRatio: 0.99,
      ssim: 0.98,
      avgDeltaE: 1.5,
      sizeMatch: true,
    });
    // clusterCheck overrides the named profile's own `cluster` default.
    expect(result.resolvedThreshold).toEqual({ ...getProfile("component/strict"), cluster: true });
    expect(result.styleGateEligible).toBe(false);
  });

  it("prefers the score's explicit styleGateEligible over the profile default", () => {
    const result = assembleContractResult({
      ...minimalInput,
      score: {
        diffPixels: 0,
    ...
    expect(result.styleGateEligible).toBe(true);
  });

  it("omits every score-derived field when the comparison never ran", () => {
    const result = assembleContractResult(minimalInput);
    expect(result.comparison).toBeUndefined();
    expect(result.resolvedThreshold).toBeUndefined();
    expect(result.styleGateEligible).toBeUndefined();
  });
});
