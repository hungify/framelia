import type { UIContractResult, UIRun } from "@framelia/contracts";

// Sized to each contract's real capture viewport (not a fixed thumbnail size) so the
// UI's pan/zoom canvas can be exercised against the same overflow conditions a
...
export const uiMockArtifacts: Record<string, string> = {
...
const contracts: UIContractResult[] = [
  {
    id: "checkout.desktop",
