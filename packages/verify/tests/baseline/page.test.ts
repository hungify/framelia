...
describe("resolvePageBaseline", () => {
  it("rejects malformed metadata without losing an accepted image on promotion", () => {
    const outDir = tempDir();
    const sourcePath = path.join(tempDir(), "capture.png");
    writePng(sourcePath, "accepted");
    const accepted = promotePageBaseline({ sourcePath, outDir, promotedBy: "alice" });
    fs.writeFileSync(pageBaselineMetaPath(outDir), "{}");
    expect(resolvePageBaseline(outDir)).toMatchObject({ ok: false, error: "BASELINE_NOT_FOUND" });
    writePng(sourcePath, "replacement");
    expect(() => promotePageBaseline({ sourcePath, outDir, promotedBy: "bob" })).toThrow(
      /metadata/i,
    );
    expect(fs.readFileSync(accepted.baselinePath, "utf8")).toBe("accepted");
  });
  it("refuses a concurrent promotion before allocating another version", () => {
    const outDir = tempDir();
    const sourcePath = path.join(tempDir(), "capture.png");
    writePng(sourcePath, "capture");
    fs.writeFileSync(path.join(outDir, ".web-baseline-promotion.lock"), "active");
    expect(() => promotePageBaseline({ sourcePath, outDir, promotedBy: "alice" })).toThrow(
      /active promotion/i,
    );
    expect(fs.existsSync(pageBaselineImagePath(outDir, 1))).toBe(false);
  });
