...
describe("contract scaffold", () => {
  it("preserves older contract documents even when force is supplied", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "framelia-scaffold-"));
    temporaryDirectories.push(directory);
    const outputPath = path.join(directory, "visual-contract.json");
    ...
    const original = JSON.stringify({
      ...request,
      contracts: [
        { ...request.contracts[0], viewport: { name: "desktop", width: 1440, height: 1024 } },
      ],
    });
    fs.writeFileSync(outputPath, original);
    expect(() => writeContractRequest(outputPath, request)).toThrow(/migrate|schema/i);
    expect(() => writeContractRequest(outputPath, request, true)).toThrow(/migrate|schema/i);
    expect(fs.readFileSync(outputPath, "utf8")).toBe(original);
  });
  it("refuses a concurrent writer instead of losing either contract", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "framelia-scaffold-"));
    temporaryDirectories.push(directory);
    const outputPath = path.join(directory, "visual-contract.json");
    const request = createContractRequest({
    ...
    fs.writeFileSync(`${outputPath}.lock`, "active");
    expect(() => writeContractRequest(outputPath, request)).toThrow(/active writer/i);
