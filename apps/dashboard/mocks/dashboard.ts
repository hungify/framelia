import { resolveDisplayThreshold } from "@framelia/contracts";
...
const contracts: DashboardContractResult[] = [
  {
    ...
    sourceRunId: "run-mock-selected",
    caseId: "checkout.desktop@chromium#0",
    contractId: "checkout.desktop",
    projectName: "chromium",
    repeatIndex: 0,
    targetPath: "/checkout",
    executionState: "completed",
    visualVerdict: "mismatched",
    selectedAttemptId: "checkout.desktop@chromium#0::attempt-1",
    provenance: {
      policyDigest: `sha256:${"1".repeat(64)}`,
      retryAcceptance: "allow-passed-after-retry",
      sourceDigest: `sha256:${"2".repeat(64)}`,
      buildDigest: `sha256:${"3".repeat(64)}`,
      dirty: false,
      bindingDigest: `sha256:${"4".repeat(64)}`,
      specFile: "tests/checkout.spec.ts",
      specFileDigest: `sha256:${"5".repeat(64)}`,
      titlePath: ["chromium", "checkout", "desktop"],
    },
    attempts: [
      {
        runId: "run-mock-selected",
        attemptId: "checkout.desktop@chromium#0::attempt-0",
        casePlanDigest: `sha256:${"6".repeat(64)}`,
        retryIndex: 0,
        selected: false,
        executionState: "completed",
        visualVerdict: "mismatched",
        evidence: {
          expected: { availability: "available", path: paths.checkout.baseline },
          actual: { availability: "available", path: paths.checkout.actual },
          diff: { availability: "available", path: paths.checkout.diff },
          score: { availability: "available", path: "mock/checkout-score-0.json" },
        },
        baseline: {
        ...
        scoreProvenance: { formatVersion: 1, digest: `sha256:${"8".repeat(64)}` },
        comparison: { matchRatio: 0.98, ssim: 0.95, avgDeltaE: 3.2, diffPixels: 8244 },
        topIssues: [],
        diagnostics: [],
        warnings: ["first attempt retained for retry history"],
      },
      {
        runId: "run-mock-selected",
        attemptId: "checkout.desktop@chromium#0::attempt-1",
        casePlanDigest: `sha256:${"6".repeat(64)}`,
        retryIndex: 1,
        selected: true,
        executionState: "completed",
        ...
        evidence: {
          ...
          actual: { availability: "available", path: paths.checkout.actual },
          diff: { availability: "available", path: paths.checkout.diff },
          score: { availability: "available", path: "mock/checkout-score-1.json" },
        },
        baseline: {
          snapshotDigest: `sha256:${"7".repeat(64)}`,
          kind: "figma",
        ...
        scoreProvenance: { formatVersion: 1, digest: `sha256:${"9".repeat(64)}` },
        comparison: { matchRatio: 0.988, ssim: 0.972, avgDeltaE: 2.31, diffPixels: 6221 },
        topIssues: [
          {
            severity: "low",
            kind: "style-color",
            message: "expected #202328ff, actual #23262bff",
    ...
    resolvedThreshold: resolveDisplayThreshold({ profile: "page", clusterCheck: true }),
    blockers: [
  ...
  },
  {
    ...
    resolvedThreshold: resolveDisplayThreshold({ profile: "component/strict" }),
    blockers: [],
