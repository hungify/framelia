...
import { projectArtifact, startUIServer } from "@framelia/ui-server";
import { afterEach, describe, expect, it } from "vitest";

import { aggregateUISource, archivedUISource, exportUIReport } from "../src/ui/report.ts";
...
async function fixture(): Promise<{
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "framelia-ui-"));
...
describe("UI projection", () => {
  ...
  it("serves archived run and only allowlisted evidence files", async () => {
    ...
    const server = await startUIServer({
      source: await archivedUISource(artifact),
      clientRoot,
    });
    try {
  ...
  it("exports portable report into empty directory", async () => {
    ...
    const indexPath = await exportUIReport({ artifact, outputDirectory, clientRoot });
  ...
  it("refuses to clear a non-empty output directory that isn't a previous Framelia report", async () => {
    ...
    await expect(exportUIReport({ artifact, outputDirectory, clientRoot })).rejects.toThrow(
  ...
  it("re-exports over its own previous report output without a caller-side rm", async () => {
    const { artifact, clientRoot, root } = await fixture();
    const outputDirectory = path.join(root, "report");
    await exportUIReport({ artifact, outputDirectory, clientRoot });
...
describe("aggregateUISource", () => {
  it("merges every visual-verification.json under .framelia/, tagging contracts with their feature", async () => {
    ...
    const source = await aggregateUISource(root);
  ...
  it("returns an empty run instead of throwing when .framelia/ has no verifications", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "framelia-ui-empty-"));
    temporaryDirectories.push(root);
    const source = await aggregateUISource(root);
    const run = await source.snapshot();
  ...
  it("ignores portable report data under report/data/ when aggregating", async () => {
    ...
    // Simulate a verify --output sibling report that stores UIRun JSON under report/data/.
