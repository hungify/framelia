...
/** A minimal but schema-valid artifact whose `ok`/`allPassed` are both `false` --
 * exercising `doneGateFromArtifact`'s "artifact itself is not ok" short-circuit
 * (see @framelia/verify's own test suite for the full score-gate pass paths;
 * this CLI-level test only needs to prove `internal/done-gate.ts`'s own thin
 * wiring -- read, options validation, config merge, `artifactPath` field). */
...
describe("doneGateCommand", () => {
  ...
  it("resolves the artifact path against the injected runtime cwd and reports done: false when the artifact itself is not ok", async () => {
    ...
    expect(result.artifactPath).toBe(artifactAbsolutePath);
    expect(result.done).toBe(false);
    expect(result.schemaVersion).toBe(SCHEMA_VERSION);
  });
});

// Note: the hidden `--max-gold-age-ms` -> `maxBaselineAgeMs` merge happens in
// `commands/done-gate.ts`'s loader, not here -- this internal function only ever
// sees the one canonical `maxBaselineAgeMs` field.
