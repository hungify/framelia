...
const pageBaselineMetaSchema = z.object({
  current: promotionSchema,
  history: z.array(promotionSchema),
});
...
export function readPageBaselineMeta(outDir: string): PageBaselineMeta | null {
  ...
  try {
    const result = pageBaselineMetaSchema.safeParse(JSON.parse(fs.readFileSync(metaPath, "utf8")));
...
export function promotePageBaseline(
  ...
  const lockPath = path.join(options.outDir, ".web-baseline-promotion.lock");
  let lock: number;
  try {
    ...
    throw new Error(
      `Baseline promotion is locked at ${lockPath}. Retry after the active promotion finishes; remove a stale lock only after confirming no promotion is running.`,
      { cause: error },
    );
  }
  try {
...
function promoteLocked(options: PromotePageBaselineOptions): PromotePageBaselineResult {
  ...
  if (!previousMeta && fs.existsSync(metaPath)) {
    throw new Error(`Invalid baseline metadata at ${metaPath}; repair it before promoting.`);
  }
  ...
  try {
    fs.copyFileSync(options.sourcePath, temporaryImage, fs.constants.COPYFILE_EXCL);
    // Linking commits complete bytes without replacing a version left by an interrupted promotion.
    fs.linkSync(temporaryImage, baselinePath);
