...
describe("framelia config boundary", () => {
  it("loads TypeScript-only syntax through the on-demand config loader", async () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), "framelia-config-typescript-"));
