...
/**
 * The prompt SEQUENCE itself -- call order, and that cancellation short-circuits
 * before any later prompt/write happens. Business-logic assertions (messages,
 * config precondition, output fields) live in `auth.test.ts`; this file exists
 * because `intro`/`confirm`/`note`/`outro` ordering is itself a documented
 * behavior contract (see the plan's Authentication acceptance contract).
 */
...
function recordingPrompts(confirmAnswers: boolean[]): {
  adapter: AuthPromptAdapter;
  calls: string[];
} {
  const calls: string[] = [];
  let confirmIndex = 0;
  const adapter: AuthPromptAdapter = {
    intro: (message) => calls.push(`intro:${message}`),
    outro: (message) => calls.push(`outro:${message}`),
    note: (message, title) => calls.push(`note:${title}`),
