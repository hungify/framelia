...
import { assertProjectRelativePath, loadEnvFiles } from "@framelia/verify/env";
...
async function importConfigModule(configPath: string): Promise<{ default?: unknown }> {
  // The config path is project-selected at runtime; tsImport handles its module format.
  const imported = (await tsImport(pathToFileURL(configPath).href, import.meta.url)) as {
    default?: unknown;
    "module.exports"?: unknown;
  };
  let value = imported.default;
  // tsx compiles .ts as CJS outside a type:module package; Node wraps its default once more.
  if (
    value &&
    typeof value === "object" &&
    "default" in value &&
